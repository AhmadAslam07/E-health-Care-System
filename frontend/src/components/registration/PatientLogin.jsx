...
const PatientLogin = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/patients/login`, {
                method: "POST",
                headers: {
            ...
            if (response.ok && data.token && data.patient) {
                localStorage.setItem("token", data.token);
                localStorage.setItem("user", JSON.stringify(data.patient));
                localStorage.setItem("userRole", "patient"); // Store user role
                navigate("/PatientDashboard/favourites");
            } else {
                        ...
                        <div className="card px-4 py-3 w-75 mx-auto shadow-sm">
                            <div className="row py-1 mb-4 text-center">
                                <div className="col-md-6">
                                    <h4 className="text-start" style={{ color: '#00205b' }}>Patient Login</h4>
                                </div>
                                <div className="col-md-6 pt-2 pe-1">
                                    <h6 className="text-end">
                                        <Link className="text-decoration-none" to="/DrsLogin" style={{ color: '#d21f49 ' }}>
                                            Login as Doctor?
                                        </Link>
                                    </h6>
                                </div>
